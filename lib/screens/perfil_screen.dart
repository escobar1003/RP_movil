...
class _PerfilScreenState extends State<PerfilScreen> {
  ...
  Future<void> _cargarDatos() async {
    ...
    int reciclajes = 0, puntos = 0, canjes = 0;
    double progreso = 0;
    try {
      final perfil = await AuthService.getPerfil();
      final dataPuntos = await AuthService.getPuntos();
      puntos = dataPuntos['saldo'] ?? dataPuntos['puntos'] ?? 0;
      reciclajes = dataPuntos['totalEntregas'] ?? dataPuntos['entregas'] ?? 0;
      canjes = dataPuntos['totalCanjes'] ?? dataPuntos['canjes'] ?? 0;
      progreso = (dataPuntos['progreso'] ?? dataPuntos['porcentaje'] ?? 0).toDouble() / 100;
    } catch (_) {}
    if (mounted) {
      setState(() {
        _nombre = nombre;
